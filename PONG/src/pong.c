...
typedef enum {
    ...
    PONG_MODE_HIGHSCORE,
    PONG_MODE_GAME,
    PONG_MODE_PAUSE          // NY: pausmeny
} PongMode_t;

static PongMode_t g_mode;

// Serve (pingis-logik) / nedräkning
...
static void pong_ai_update(void)
{
  ...
  if (g_diff == PONG_DIFF_HARD) {
    ...
    target_y = g_state.ball.y + (rand() % 5 - 2); // liten slumpmässig felmarginal
} else {
...
static void pong_render(void)
{
    ...
    //    – rensa en bred ruta över mitten så all text försvinner helt.
    int cx1 = 0;
    ...
    if (g_phase == PONG_PHASE_SERVE && g_serve_count > 0) {
        // Om vi tidigare ritade "winner", rensa den rutan en gång
        if (g_prev_winner_drawn) {
            LCD_Fill(cx1, cy1, cx2, cy2, BLACK);
            g_prev_winner_drawn = 0;
        }

        // Rita ny siffra bara när värdet ändras
        ...
        // Ingen nedräkning -> nolla ev gammalt countdown-state
        ...
        // Varken serve-countdown eller winner → se till att rutan är tom om något fanns
...
static void draw_main_menu(void)
{
    ...
    LCD_ShowString(60, 5, (u8*)"PONG", WHITE);
    ...
    for (int i = 0; i < 3; ++i) {
        int y = 25 + i * 12;
        if (i == g_menu_index) {
            LCD_ShowString(10, y, (u8*)">", WHITE);
        }
        LCD_ShowString(20, y, (u8*)items[i], WHITE);
    }
...
static void draw_diff_menu(void)
{
    ...
    BACK_COLOR = BLACK;
    LCD_Clear(BLACK);
    LCD_ShowString(10, 5, (u8*)"CHOOSE DIFFICULTY", WHITE);

    const char *items[2] = { "EASY", "HARD" };
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        LCD_ShowString(25, y, (u8*)items[i], WHITE);
    }
...
static void draw_pause_menu(void)
{
    ...
    LCD_ShowString(40, 5, (u8*)"PAUSED", WHITE);

    // Radtexter – diff-raden beror på aktuell svårighetsgrad
    const char *item0 = "RESUME GAME";
    const char *item2 = "MAIN MENU";
    const char *item1 = (g_diff == PONG_DIFF_EASY)
                        ? "DIFFICULTY: EASY"
                        : "DIFFICULTY: HARD";

    const char *items[3] = { item0, item1, item2 };
    ...
    LCD_Wait_On_Queue();   // se till att ALL text kommer ut
...
void vPongTask(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        case PONG_MODE_MENU:
            ...
            if (fire_edge) {
                if (g_menu_index == 0) {
                    ...
                    vTaskDelete(NULL);  // TODO: senare kan ni hoppa till “console menu”-task
                }
            }
        ...
        case PONG_MODE_HIGHSCORE:
            ...
            // highscore-skärmen är statisk annars
            break;
        ...
        case PONG_MODE_PAUSE:
            ...
            if (fire_edge) {
                if (g_pause_index == 0) {
                    ...
                    g_mode = PONG_MODE_GAME;
                    // pong_update/render körs igen i nästa varv
                }
                    ...
                    // tvinga omritning så texten DIFFICULTY: ... uppdateras
                    g_prev_pause_index = -1;
                }
